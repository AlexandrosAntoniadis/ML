...
directory_name = inst+'EWmyresults'
...
np.savetxt(inst+"EW.dat", table, header = headers, delimiter=",")   
    
     # transpose
     
table = np.loadtxt(inst+'EW.dat', dtype=str, delimiter=',', comments="?")

table_T = np.transpose(table)
table_T[0,0] = table_T[0,0].replace('# ','')
print(table_T[0])
np.savetxt(inst+'cleanstars.csv', table_T, delimiter=',', fmt='%s')
